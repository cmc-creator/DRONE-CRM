      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black tracking-wide"
            style={{ background: "linear-gradient(135deg,#fff 0%,#00d4ff 60%,#7c3aed 100%)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text" }}>
            Mission Calendar
          </h1>
          <p className="text-xs mt-0.5" style={{ color: "rgba(0,212,255,0.4)" }}>Scheduled flights & operations</p>
        </div>
        <Link href="/admin/jobs/new">
          <button className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all"
            style={{ background:"rgba(0,212,255,0.1)", border:"1px solid rgba(0,212,255,0.25)", color:"#00d4ff" }}>
            <CalendarDays className="w-4 h-4" />Schedule Mission
          </button>
        </Link>
      </div>

      <div className="rounded-2xl overflow-hidden" style={{ background:"rgba(255,255,255,0.02)", border:"1px solid rgba(0,212,255,0.08)" }}>
        <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom:"1px solid rgba(0,212,255,0.07)" }}>
          <Link href={`/admin/calendar?month=${prevMonth.m}&year=${prevMonth.y}`}>
            <button className="p-2 rounded-lg transition-all hover:bg-cyan-900/30" style={{ color:"rgba(0,212,255,0.6)" }}>
        ...
        <div className="grid grid-cols-7 text-center">
          {DAY_LABELS.map((d) => (
            <div key={d} className="py-3 text-xs font-bold uppercase tracking-wider" style={{ color:"rgba(0,212,255,0.35)" }}>
              {d}
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-7">
          {Array.from({ length: firstWeekDay }).map((_, i) => (
            <div key={`empty-${i}`} className="min-h-24 p-2" style={{ borderTop:"1px solid rgba(0,212,255,0.05)", borderRight:"1px solid rgba(0,212,255,0.05)" }} />
          ))}
            ...
            return (
              ...
              >
                ...
                <div className="space-y-1">
                  ...
                  {dayJobs.length > 3 && (
                    <p className="text-[10px]" style={{ color:"rgba(0,212,255,0.3)" }}>+{dayJobs.length - 3} more</p>
                  )}
                </div>
              </div>
            );
          ...
          {Array.from({ length: (7 - ((firstWeekDay + daysInMonth) % 7)) % 7 }).map((_,i) => (
            <div key={`trail-${i}`} className="min-h-24 p-2" style={{ borderTop:"1px solid rgba(0,212,255,0.05)", borderRight:"1px solid rgba(0,212,255,0.05)" }} />
          ))}
        </div>
      </div>
      ...
      {jobs.length > 0 && (
        <div className="rounded-2xl overflow-hidden" style={{ background:"rgba(255,255,255,0.02)", border:"1px solid rgba(0,212,255,0.08)" }}>
          <div className="px-5 py-3" style={{ borderBottom:"1px solid rgba(0,212,255,0.07)" }}>
            <p className="text-xs font-bold uppercase tracking-wider" style={{ color:"rgba(0,212,255,0.5)" }}>
                ...
                <Link key={job.id} href={`/admin/jobs/${job.id}`}>
                  <div className="flex items-center gap-4 px-5 py-3 transition-colors hover:bg-cyan-900/10 cursor-pointer">
                    <div className="w-10 text-center">
                      <p className="text-xl font-black leading-none" style={{ color:"#00d4ff" }}>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold truncate" style={{ color:"#d8e8f4" }}>{job.title}</p>
                      <p className="text-xs truncate" style={{ color:"rgba(0,212,255,0.4)" }}>
                        {job.client?.companyName ?? "No client"}{pilotName ? ` · ${pilotName}` : " · Unassigned"}
                      </p>
                    </div>
                    <span className="text-[11px] font-bold px-2 py-1 rounded-full whitespace-nowrap"
                      style={{ background:`${STATUS_COLORS[job.status] ?? "#94a3b8"}15`, color:STATUS_COLORS[job.status] ?? "#94a3b8" }}>
